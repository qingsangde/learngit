...
{
    public class sta_DayTestDriveAnalyse
    {
        public ResponseResult getDayAnalyseList(Dictionary<string, string> inparams)
        {
            ...
            try
            {
                ...
                {   /****************************生成未行转列的表格数据*******************************/
                    ...
                    {
                        rr["TDATE"] = rr["TEST_DRIVE_DATE"].ToString().Substring(7, 2) + "日";
                    ...
                    foreach (DataRow rr in rs.Rows) {
                        ...
                        if (rr["TDATE"].ToString().Equals("合计"))
                            ...
                            colName = rr["TEST_DRIVE_DATE"].ToString().Substring(0,10).Replace("/", "");
                    ...
                    foreach (DataRow rr in rs.Rows) {
                        ...
                        if (rr["TDATE"].ToString().Equals("合计"))
                            ...
                            colName = rr["TEST_DRIVE_DATE"].ToString().Substring(0, 10).Replace("/", "");
                        //newDr1["TITLE"] = "时间/日期";
                        //newDr1[colName] = rr["TDATE"].ToString();
                        newDr2["TITLE"] = "试乘试驾次数";
        ...
        public DataTable getList(string key, string CIDs, string STime)
        {
            ...
            try
            {
                ...
                DataTable dt = new DataTable();
                dt = oSqlUtil.FillDataSet(key, WebProc.Proc("X80Proc_DayAnalyse_List_Select"), oaPara, "fencetable", 30).Tables[0];
                return dt;
            }
            catch (Exception ex)
            {
        ...
        public ResponseResult getDealerName(Dictionary<string, string> inparams)
        {
            ResponseResult Result = null;
            ComSqlHelper oSqlUtil = new ComSqlHelper();
            string sysflag;
            ...
            try
            {
                ...
                oaPara[0] = new SqlParameter("@Type", Type);
                oaPara[1] = new SqlParameter("@Area", Area);
                oaPara[2] = new SqlParameter("@Province", Province);
                DataTable dt = new DataTable();
                dt = oSqlUtil.FillDataSet(sysflag, WebProc.Proc("X80Proc_DayAnalyse_DealerName_Get"), oaPara, "fencetable", 30).Tables[0];
        ...
        public ResponseResult doExport(Dictionary<string, string> inparams)
        {
            ...
            try
            {
                ...
                string[] headerDataArray = { "销售大区", "销售省区", "经销商代码", "经销商名称", "车型", "车牌号" ,
                                           ...
                                           "试乘试驾日期", "试乘试驾次数", "试乘试驾里程", "总行驶里程", "试乘试驾里程占比", "试乘试驾时间", "总行驶时间" ,
            ...
            string dealername, string dealercode, string cartype, string carno)
        {
            ...
            return csh.FillDataSet(sysflag, WebProc.Proc("X80Proc_TestDriveDayDataExport"), Parameters, null, 1800).Tables[0];
