...
namespace SysService
{
    public class sta_outdrive
    {
        ...
        /// <returns></returns>
        public ResponseResult getOCDetail(Dictionary<string, string> inparams)
        {
            ...
            try
            {
                ...
                //调用存储过程查询试乘试驾数据
                //todo
                DataTable dt = GetOutDriveSituation(sysflag, cid, stime, etime);
        ...
        /// <param name="inparams"></param>
        /// <returns></returns>
        public ResponseResult getOCOutput(Dictionary<string, string> inparams)
        {
            ResponseResult Result = null;
            string stime;
            string etime;
            ...
            try
            {
                ...
                //调用存储过程查询启动熄火车辆列表
                //todo
                DataTable contentData = GetOutDriveSituation(sysflag, cid, stime, etime);
                ...
                npoiHelper.WorkbookName = "偏离试驾路线统计" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
                // 设置导入内容
