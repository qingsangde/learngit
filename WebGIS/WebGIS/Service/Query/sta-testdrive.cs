...
namespace SysService
{
    public class sta_testdrive
    {
        ...
        /// <returns></returns>
        public static DataTable GetTestSituation(string sysflag, string cid, string stime, string etime)
        {
            ...
            return csh.FillDataSet(sysflag, WebProc.Proc("X80Proc_XP_TestDriveInfo"), Parameters, null, 1800).Tables[0];
        ...
        /// <param name="inparams"></param>
        /// <returns></returns>
        public ResponseResult getTCOutput(Dictionary<string, string> inparams)
        {
            ResponseResult Result = null;
            string stime;
            string etime;
            ...
            try
            {
                ...
                //调用存储过程查询启动熄火车辆列表
                //todo
                DataTable contentData = GetTestSituation(sysflag, cid, stime, etime);

                NPOIHelper npoiHelper = new NPOIHelper();
                string[] headerDataArray = { "序号", "车牌号", "开始时间-结束时间", "运行时长", "运行里程", "开始位置", "结束位置" };
                string[][] contentDataArray = npoiHelper.convertDataTableToStringArray(contentData);
                npoiHelper.WorkbookName = "试乘试驾查询统计" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
                // 设置导入内容
