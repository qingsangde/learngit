        ...
        if (obj.state == 100) {
            ...
            $('#ff').combogrid({
                onClickRow: function (rowIndex, rowData) {
                    ...
                    $('#spanFname')[0].innerHTML = rowData.Name;
                    $('#spanGType')[0].innerHTML = rowData.GType;
                    $('#spanAlmType')[0].innerHTML = rowData.AlmType;
                    $('#spanDesc')[0].innerHTML = rowData.Desc;
                    $("#spanFence").show();
                    $("#divFilter").hide();
                    DrawGraph(rowData.Gragh, rowData.Con);
                    BindCarList();
                }
            });
        }
...
function DrawGraph(graphtype, content) {
    if (graphtype == 1) { //圆形
        addCircle(content); //在地图上画出区域
    }
    else {//多边形
        addPolygon(content); //在地图上画出区域
    }
}
...
function BindCarList() {
    ...
    //$.post("GetFenceCar.ashx", { key: key, uid: uid, fid: fid }, setCarData);
}
...
function setCarData(obj) {
    if (obj != null) {
        if (obj.state == 100) {
            ...
            BindGridData();
            //        if (data.FenceCars.length > 0) {
            //            var fencecars = data.FenceCars;
            //            for (var i = 0; i < fencecars.length; i++) {
            //                var tno = fencecars[i].TNO;
            //                selecttnos.push(tno);
            //            }
            //        }
        }
