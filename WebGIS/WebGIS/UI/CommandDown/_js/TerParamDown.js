...
function sendTimeIntervalOrder() {
    cids = [];
    tnos = [];
    carnos = [];
    if (CarArr.length > 0) {
        ...
        var mydata = {
            ...
            "tnos": tnos.join(","),
            "carnos": carnos.join(","),
            "timeinterval": $('#txtSendTimeInterval').val()
        };
        BaseGetData(mydata, SendTimeIntervalRes);
    }
    else {
...
function SendTimeIntervalRes(res) {
    if (res.state == 100) {
        var resstr = "";
